...
const Placeholder = ( { clientId, name, setAttributes } ) => {
	...
	useEffect( () => {
		const skipButton = document.querySelector(
			'.wp-block-gutena-forms .block-editor-block-variation-picker__skip'
		);

		if ( skipButton ) {
			const textNode = Array.from( skipButton.childNodes ).find(
				( node ) => {
					return (
						node.nodeType === Node.TEXT_NODE ||
						( node.nodeType === Node.ELEMENT_NODE &&
							node.textContent?.trim() === 'Skip' )
					);
				}
			);
			...
			if ( textNode ) {
				if ( textNode.nodeType === Node.TEXT_NODE ) {
					textNode.textContent = textNode.textContent.replace(
						'Skip',
						__( 'Continue with default', 'gutena-forms' )
					);
				} else {
					textNode.textContent = __(
				...
				if ( skipText.includes( 'Skip' ) ) {
					skipButton.textContent = skipText.replace(
						'Skip',
						__( 'Continue with default', 'gutena-forms' )
					);
				}
...
export default function Edit( props ) {
	...
	const getEmailFields = () => {
		...
		if ( ! gfIsEmpty( blocks ) ) {
			let emailFields = getInnerBlocksbyNameAttr(
				blocks.innerBlocks,
				'gutena/form-field',
				'fieldType',
				'email'
			);
			let standaloneEmailFields = getInnerBlocksbyNameAttr(
				blocks.innerBlocks,
				'gutena/email-field'
			);
			...
			if ( 0 < emailFields.length ) {
				for ( let i = 0; i < emailFields.length; i++ ) {
					...
					const isStandaloneEmail =
						'gutena/email-field' === emailFields[ i ].name;
					if (
						! gfIsEmpty( emailAttr.nameAttr ) &&
						( isStandaloneEmail || 'email' === emailAttr.fieldType )
					) {
						emailOptions.push( {
							label: emailAttr.fieldName,
							value: emailAttr.nameAttr,
						} );
					}
				}
			}
	...
	const getTextFields = () => {
		...
		const blocks = getBlock( clientId );
		if ( ! gfIsEmpty( blocks ) ) {
			let textFields = getInnerBlocksbyNameAttr(
				blocks.innerBlocks,
				'gutena/form-field',
				'fieldType',
				'text'
			);
			let standaloneTextFields = getInnerBlocksbyNameAttr(
				blocks.innerBlocks,
				'gutena/text-field'
			);
			...
			if ( 0 < textFields.length ) {
				for ( let i = 0; i < textFields.length; i++ ) {
					...
					if (
						! gfIsEmpty( fieldAttr.nameAttr ) &&
						( 'text' === fieldAttr.fieldType ||
							gfIsEmpty( fieldAttr.fieldType ) )
					) {
						textOptions.push( {
							label: fieldAttr.fieldName,
							value: fieldAttr.nameAttr,
						} );
					}
				}
			}
	...
	useEffect( () => {
		...
		if ( shouldRunFormID && gfIsEmpty( formID ) ) {
			...
			if (
				! gfIsEmpty( recaptcha ) &&
				gfIsEmpty( recaptcha.secret_key ) &&
				! gfIsEmpty( gutenaFormsBlock ) &&
				! gfIsEmpty( gutenaFormsBlock.grecaptcha_type ) &&
				! gfIsEmpty( gutenaFormsBlock.grecaptcha_site_key ) &&
				! gfIsEmpty( gutenaFormsBlock.grecaptcha_secret_key )
			) {
				setAttributes( {
					recaptcha: {
						...recaptcha,
		...
		if (
			shouldRunFormID &&
			gfIsEmpty( replyToEmail ) &&
			gfIsEmpty( replyToName )
		) {
			let emailOptions = getEmailFields();
			let textOptions = getTextFields();
			if (
				1 < emailOptions.length &&
				! gfIsEmpty( emailOptions[ 1 ].value ) &&
				1 < textOptions.length &&
				! gfIsEmpty( textOptions[ 1 ].value )
			) {
				setAttributes( {
	...
	const hasExistingFormsBlock = useSelect(
		( select ) => {
			...
			const existingFormsBlocks = getInnerBlocksbyNameAttr(
				blocks,
				'gutena/existing-forms'
			);
			return (
				! gfIsEmpty( existingFormsBlocks ) &&
				existingFormsBlocks.length > 0
			);
		},
		[ clientId ]
	);
	...
	useEffect( () => {
		...
		if ( shouldRunFormClassnames ) {
			...
			formClassName += afterSubmitAction
				...
				: '';
			formClassName += gfIsEmpty( labelTypography?.fontFamily )
				? ''
				: ' has-label-font-family';
			formClassName += gfIsEmpty( placeholderTypography?.fontFamily )
				? ''
	...
	const PRO_STANDALONE_BLOCK_NAMES = [
		'gutena/url-field',
		'gutena/phone-field',
				...
				<InspectorControls>
					<PanelBody title="Form settings" initialOpen={ true }>
						...
						<p>
							<span className="block-editor-block-card__title">
								{ __( 'Note : ', 'gutena-forms' ) }
							</span>
							<span className="gf-text-muted">
								<span>
									{ __(
										'To reuse this form, please make it a ',
										'gutena-forms'
									) }
								</span>
								<a
									href="https://gutenaforms.com/reuse-gutena-forms-on-multiple-pages"
									target="_blank"
								>
								...
								<span>
									{ __(
										' ( Reusable Block ). Avoid copying or duplicating this block.',
										'gutena-forms'
									) }
								</span>
							</span>
						...
						<VStack>
							<p
								dangerouslySetInnerHTML={ {
									__html: sprintf(
										__(
											'Enable %1$s v2 or v3 to protect your forms from spam and bot submissions.',
											'gutena-forms'
										),
										`<a href="https://gutenaforms.com/how-to-generate-google-recaptcha-site-key-and-secret-key" target="_blank">${ __(
											'reCAPTCHA',
											'gutena-forms'
										) }</a>`
									),
						...
						<VStack>
							<p
								dangerouslySetInnerHTML={ {
									__html: sprintf(
										__(
											'Enable %1$s to protect your forms from spam and bot submissions.',
											'gutena-forms'
										),
										`<a href="https://developers.cloudflare.com/turnstile/get-started/">${ __(
											'Cloudflare CAPTCHA',
											'gutena-forms'
										) }</a>`
									),
							...
							{ ! gfIsEmpty( cloudflareTurnstile?.enable ) &&
								cloudflareTurnstile?.enable && (
									<>
										...
										<TextControl
											...
											value={
												cloudflareTurnstile?.secret_key
											}
											onChange={ ( secret_key ) =>
												setAttributes( {
													cloudflareTurnstile: {
								...
								honeypot?.enable && (
									<>
										<NumberControl
											...
											description={
												'Adds a time-based spam check that detects submissions made too quickly, a common bot behavior. The default threshold is 4 seconds, adjustable as needed. If unsure, leave it off.'
											}
										/>
									</>
								) }
					...
					>
						...
						<RangeControlUnit
							...
							onChangeFunc={ ( inputBorderRadius ) =>
					...
					<PanelBody
						title={ __( 'Messages', 'gutena-forms' ) }
						initialOpen={ false }
					>
						<TextControl
							type="text"
							...
							value={
								gfIsEmpty( messages?.required_msg )
									? ''
						...
						<TextControl
							...
							value={
								gfIsEmpty( messages?.required_msg_select )
									? ''
									: messages?.required_msg_select
							}
							...
							placeholder={
								gutenaFormsBlock?.required_msg_select
							}
						/>
						<TextControl
							type="text"
							...
							value={
								gfIsEmpty( messages?.required_msg_check )
									? ''
									: messages?.required_msg_check
							}
						...
						<TextControl
							...
							value={
								gfIsEmpty( messages?.required_msg_optin )
									? ''
									: messages?.required_msg_optin
							}
						...
						<TextControl
							...
							value={
								gfIsEmpty( messages?.invalid_email_msg )
									? ''
									: messages?.invalid_email_msg
							}
						...
						<TextControl
							...
							value={
								gfIsEmpty( messages?.min_value_msg )
									? ''
									: messages?.min_value_msg
							}
						...
						<TextControl
							...
							value={
								gfIsEmpty( messages?.max_value_msg )
									? ''
									: messages?.max_value_msg
							}
