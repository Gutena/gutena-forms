...
function getFieldClasses( { isRequired, autocomplete } ) {
	const parts = [ 'gutena-forms-field', 'select-field', 'state-field' ];
	if ( isRequired ) {
...
export default function Edit( { attributes, setAttributes, clientId } ) {
	...
	const blockProps = useBlockProps( {
		className:
			'wp-block-gutena-field-group wp-block-gutena-state-field field-group-type-select standalone-state-field',
	} );
		...
		<>
			...
			<div { ...blockProps }>
				...
				{ ! gfIsEmpty( description ) && (
					<p className="gutena-forms-state-field-description">
						{ description }
					</p>
				) }
