...
export default function Edit( { attributes, setAttributes, clientId } ) {
	...
	const blockProps = useBlockProps( {
		className: 'wp-block-gutena-field-group wp-block-gutena-url-field field-group-type-url standalone-url-field',
	} );
		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Field settings', 'gutena-forms' ) } initialOpen={ true }>
					...
					<RangeControl
						...
						value={ maxlength ?? 0 }
						onChange={ ( v ) => setAttributes( { maxlength: v } ) }
						min={ 0 }
						max={ 500 }
						step={ 25 }
					/>
			...
			<div { ...blockProps }>
				...
				<input
					...
					name={ nameAttr }
					type="url"
					className={ `gutena-forms-field url-field ${ isRequired ? 'required-field' : '' } ${ autocomplete ? 'autocomplete' : '' }` }
					placeholder={ placeholder || 'https://' }
					defaultValue={ defaultValue }
					maxLength={ maxlength && maxlength > 0 ? maxlength : undefined }
					readOnly
				/>
				{ ! gfIsEmpty( description ) && <p className="gutena-forms-url-field-description">{ description }</p> }
