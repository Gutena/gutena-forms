										...
										if ( $has_paid_plan || $has_bundle_license ) {
											if ( $fs->is_trial() ) {
                                                ...
                                                if ( $show_plan_row ) {
                                                    $profile[] = array(
                                                        'id'    => 'plan',
                                                        'title' => ( $has_bundle_license ? ucfirst( $fs->get_module_type() ) . ' account.php' : '' ) . $plan_text,
													...
													<?php if ( 'plan' === $p['id'] ) : ?>
														...
														<?php if ( is_object( $license ) && ! $license->is_lifetime() ) : ?>
															<?php if ( ! $is_active_subscription && ! $is_active_bundle_subscription && ! $license->is_first_payment_pending() ) : ?>
                                                                ...
                                                                <?php $expired_ago_text   = ( fs_text_inline( 'Expired', 'expired', $slug ) . ' account.php' . fs_text_x_inline( '%s ago', 'x-ago', $slug ) ) ?>
																<label
																	...
																	<?php if ( $is_paying || $fs->is_trial() ) : ?>
																		<?php if ( ! $fs->is_allowed_to_install() ) : ?>
                                                                            <a target="_blank" rel="noopener" class="button button-primary"
                                                                                ...
                                                                                $download_version_text = account . phpsprintf(
                                                                                ...
                                                                                if ( $download_version_text_length > 31 ) {
                                                                                     ...
                                                                                     * @aince 2.3.2
                                                                                     */
                                                                                    $download_version_text = account . phpfs_text_inline( 'Download Paid Version', 'download-paid-version', $slug ) . $download_version_text_suffix;
                                                                                }
