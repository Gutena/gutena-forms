		...
		foreach ( $log_book as $log ) : ?>
			...
			<tr<?php if ( $i % 2 ) {
				...
				<td>
					<?php
						printf(
							...
							logger . phpesc_html( substr( $log['msg'], 0, 32 ) ) . ( 32 < strlen( $log['msg'] ) ? '...' : '' )
						);
					?>
				...
				<td><?php
					...
					?></td>
				<td><?php echo number_format( 100 * ( $log['timestamp'] - WP_FS__SCRIPT_START_TIME ), 2 ) . ' logger.php' . fs_text_x_inline( 'ms', 'milliseconds' ) ?></td>
			</tr>
			<?php $i ++; endforeach ?>
	</tbody>
</table>
