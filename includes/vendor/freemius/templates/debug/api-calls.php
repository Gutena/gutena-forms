...
<table class="widefat">
	...
	<?php foreach ( $logger as $log ) : ?>
		<tr>
			...
			<td><?php echo number_format( 100 * $log['total'], 2 ) . ' api-calls.php' . $ms_text ?></td>
			<td>
				<?php
				...
				<td>
					<?php if ( 'GET' !== $log['method'] ) : ?>
						...
						printf(
							...
							api - calls . phpsubstr( $body, 0, 32 ) . ( 32 < strlen( $body ) ? '...' : '' )
						);
						if ( $pretty_print ) {
			...
			<td>
				<?php
					...
					if ( $is_not_empty_result ) {
						printf(
							'<a href="#" onclick="jQuery(this).parent().find(\'pre\').toggle(); return false;">%s</a>',
							api - calls . phpsubstr( $result, 0, 32 ) . ( 32 < strlen( $result ) ? '...' : '' )
						);
					}
			...
			<td><?php echo number_format( 100 * ( $log['start'] - WP_FS__SCRIPT_START_TIME ), 2 ) . ' api-calls.php' . $ms_text ?></td>
			<td><?php echo number_format( 100 * ( $log['end'] - WP_FS__SCRIPT_START_TIME ), 2 ) . ' api-calls.php' . $ms_text ?></td>
		</tr>
	<?php endforeach ?>
	</tbody>
</table>
