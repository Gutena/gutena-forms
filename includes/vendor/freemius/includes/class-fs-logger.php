...
KEY `type` (`type` ASC))" );
		...
		public static function download_db_logs(
			...
			$filepath   = rtrim( $upload_dir['path'], '/' ) . "/class-fs-logger.php";
			...
			return rtrim( $upload_dir['url'], '/' ) . 'class-fs-logger.php/' . $filename;
		}
		...
		public static function get_logs_download_url( $filename = '' ) {
			...
			return - fs - logger . phprtrim( $upload_dir['url'], '/' ) . $filename;
		}
