	...
	abstract class Gutena_Forms_Payment_Gateway_Settings extends Gutena_Forms_Forms_Settings {
		...
		public function save_settings( $settings ) {
			...
			if ( class_exists( 'Gutena_Forms_Stripe_Connect' ) ) {
				if ( array_key_exists( 'publishable_key_test', $merged ) ) {
					$sanitized['publishable_key_test'] = Gutena_Forms_Stripe_Connect::sanitize_publishable_key( $merged['publishable_key_test'] );
				} elseif ( ! empty( $previous['publishable_key_test'] ) ) {
					$sanitized['publishable_key_test'] = $previous['publishable_key_test'];
				}

				if ( array_key_exists( 'publishable_key_live', $merged ) ) {
					$sanitized['publishable_key_live'] = Gutena_Forms_Stripe_Connect::sanitize_publishable_key( $merged['publishable_key_live'] );
				} elseif ( ! empty( $previous['publishable_key_live'] ) ) {
					$sanitized['publishable_key_live'] = $previous['publishable_key_live'];
				}
			}
