...
if ( ! class_exists( 'Gutena_Forms_Entries_Model' ) ) :
	...
	public function get_all( $form_id = 0 ) {
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- placeholders are filled via $wpdb->prepare() below.
		$query = "SELECT e.entry_id, e.form_id, f.form_name, e.added_time, e.entry_data, e.entry_status AS status, m.metadata AS starred FROM %i e LEFT JOIN %i f ON e.form_id = f.form_id LEFT JOIN %i m ON e.entry_id = m.entry_id AND m.data_type = 'starred' WHERE e.trash = 0 ";
		$args   = array(
			$this->store->table_gutenaforms_entries,
			$this->store->table_gutenaforms,
