 ...
 if ( ! class_exists( 'Gutena_Forms_Manage_Store' ) && class_exists( 'Gutena_Forms_Store' ) ) {
	...
	class Gutena_Forms_Manage_Store extends Gutena_Forms_Store {
		...
		protected function add_form_or_enries_meta( $meta = array() ) {
			...
			global $wpdb; 
			if ( empty( $wpdb ) || Gutena_Forms_Admin_Helper::is_empty( $meta, array( 'form_id', 'data_type', 'metadata'  ) ) ) {
				return;
			}
			...
			$wpdb->insert(
				...
				array(
					...
					'metadata' => Gutena_Forms_Admin_Helper::sanitize_serialize_data( $meta['metadata'] ),
				),
				array(
					'%d',
					'%d',
		...
		public function save_form_schema( $form_schema, $block_form_id, $gutena_form_ids ) {
			...
			if (  empty( $fom_schema_row ) ) {
				...
				$form_schema_serialize = Gutena_Forms_Admin_Helper::sanitize_serialize_data( $form_schema );
			...
			if ( ! empty( $fom_schema_row ) ) {
				...
				$wpdb->insert(
					...
					array(
						...
						'entry_data' => Gutena_Forms_Admin_Helper::sanitize_serialize_data( $form_data ),
					),
					array(
						'%d',
						'%d',
				...
				$this->add_form_or_enries_meta( array(
					...
					'metadata' => Gutena_Forms_Admin_Helper::sanitize_serialize_data( $form_data ),
				) );
