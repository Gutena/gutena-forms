...
if ( ! class_exists( 'Gutena_Forms_Store' ) && class_exists( 'Gutena_Forms_Admin' ) ) {
	...
	class Gutena_Forms_Store extends Gutena_Forms_Admin {
		...
		public function view_field_value( $fieldData, $fieldvalue = '' ) {
			return ( empty( $fieldData ) || empty( $fieldData['value'] ) ) ? '' : wp_kses(
				apply_filters(
		...
		public function get_dashboard_header() {
			...
			return '<div class="gf-header">
				...
				<div class="pr-16">
					' . ( Gutena_Forms_Helper::has_pro() ? '' : '<a href="https://gutenaforms.com/pricing/?utm_source=plugin_dashboard&utm_medium=website&utm_campaign=free_plugin" target="_blank" rel="noopener noreferrer" class="gf-btn gf-pro-btn"  > <span class="gf-btn-text">' . __( 'Go Premium', 'gutena-forms' ) . '</span> </a>' ) . '
				</div>
			</div>';
		}
	}
