...
if ( ! class_exists( 'Gutena_Forms' ) ) {
	...
	class Gutena_Forms {
		...
		public function register_rest_routes() {
			register_rest_route( 'gutena-forms/v1', '/forms', array(
				'methods' => 'GET',
				'callback' => array( $this, 'get_forms_rest' ),
				'permission_callback' => function() {
					return current_user_can( 'edit_posts' );
				}
			) );

			register_rest_route( 'gutena-forms/v1', '/forms/(?P<id>\d+)', array(
				'methods' => 'GET',
				'callback' => array( $this, 'get_form_rest' ),
				'permission_callback' => function() {
					return current_user_can( 'edit_posts' );
				},
				'args' => array(
			...
			register_rest_route( 'gutena-forms/v1', '/forms/(?P<id>\d+)/update', array(
				'methods' => 'POST',
				'callback' => array( $this, 'update_form_rest' ),
				'permission_callback' => function() {
					return current_user_can( 'edit_posts' );
				},
				'args' => array(
		...
		// Get forms for REST API
		public function get_forms_rest( $request ) {
		...
		// Get individual form for REST API
		public function get_form_rest( $request ) {
			$form_id = $request['id'];
			$form = get_post( $form_id );

			if ( ! $form || 'gutena_form' !== $form->post_type ) {
				return new WP_Error( 'form_not_found', __( 'Form not found.', 'gutena-forms' ), array( 'status' => 404 ) );
			}

			// Parse the form content to extract attributes
			$blocks = parse_blocks( $form->post_content );
			$form_attributes = array();
			
			if ( ! empty( $blocks ) ) {
				$form_block = $blocks[0];
				if ( isset( $form_block['blockName'] ) && 'gutena/forms' === $form_block['blockName'] ) {
		...
		// Update form for REST API (without triggering save_post hooks)
		public function update_form_rest( $request ) {
			$form_id = $request['id'];
			...
			// Temporarily remove our save_post hooks to prevent infinite loops
			...
			if ( $result === false ) {
				return new WP_Error( 'update_failed', __( 'Failed to update form.', 'gutena-forms' ), array( 'status' => 500 ) );
			}

			// Clear any caches
			clean_post_cache( $form_id );

			// Sync changes across all instances (with error handling)
			// Temporarily disable sync to prevent server errors
			$enable_sync = apply_filters( 'gutena_forms_enable_sync', false );
			if ( $enable_sync ) {
				try {
					$helper = new Gutena_Helper();
					$helper->sync_form_changes( $form_id, $content );
				} catch ( Exception $e ) {
					// Log sync error but don't fail the main update
					error_log( 'Gutena Forms Sync Error in REST API: ' . $e->getMessage() );
				}
			}

			// Return updated form data
			$updated_form = get_post( $form_id );
			$form_data = array(
				'id' => $updated_form->ID,
				'title' => $updated_form->post_title,
				'content' => $updated_form->post_content,
				'date' => $updated_form->post_date
			);
		...
		public function register_blocks_and_scripts() {
			...
			// Existing Forms Block
			register_block_type(
				__DIR__ . '/build/existing-forms',
				array(
					'render_callback' => array( $this, 'render_existing_forms' ),
					'attributes' => array(
						'formId' => array(
							'type' => 'number',
							'default' => 0
		...
		// render_callback : existing forms
		public function render_existing_forms( $attributes, $content, $block ) {
			// Prevent infinite loops by tracking rendered forms
			static $rendering_forms = array();
			
			// Check memory usage to prevent exhaustion
			$memory_usage = memory_get_usage( true );
			$memory_limit = ini_get( 'memory_limit' );
			$memory_limit_bytes = is_numeric( $memory_limit ) ? $memory_limit : wp_convert_hr_to_bytes( $memory_limit );
			
			// If we're using more than 80% of available memory, stop rendering
			if ( $memory_usage > ( $memory_limit_bytes * 0.8 ) ) {
				return '<p class="gutena-forms-error">' . __( 'Memory limit reached. Form cannot be rendered.', 'gutena-forms' ) . '</p>';
			}
			
			$form_id = isset( $attributes['formId'] ) ? intval( $attributes['formId'] ) : 0;
			
			if ( ! $form_id ) {
				return '<p class="gutena-forms-error">' . __( 'No form selected.', 'gutena-forms' ) . '</p>';
			}

			// Check if we're already rendering this form to prevent infinite loops
			if ( isset( $rendering_forms[ $form_id ] ) ) {
				return '<p class="gutena-forms-error">' . __( 'Form is already being rendered to prevent infinite loop.', 'gutena-forms' ) . '</p>';
			}

			$form_post = get_post( $form_id );
			if ( ! $form_post || 'gutena_form' !== $form_post->post_type ) {
				return '<p class="gutena-forms-error">' . __( 'Selected form not found.', 'gutena-forms' ) . '</p>';
			}

			// Track usage of this form for synchronization
			$this->track_form_usage( $form_id );

			// Mark this form as being rendered
			$rendering_forms[ $form_id ] = true;

			try {
				// Check content size to prevent memory issues
				$content_size = strlen( $form_post->post_content );
				$max_content_size = 1024 * 1024; // 1MB limit
				
				if ( $content_size > $max_content_size ) {
					unset( $rendering_forms[ $form_id ] );
					return '<p class="gutena-forms-error">' . __( 'Form content is too large to render safely.', 'gutena-forms' ) . '</p>';
				}
				
				// Render the form content with error handling
				$rendered_content = apply_filters( 'the_content', $form_post->post_content );
			} catch ( Exception $e ) {
				// Remove from rendering list on error
				unset( $rendering_forms[ $form_id ] );
				return '<p class="gutena-forms-error">' . __( 'Error rendering form: ', 'gutena-forms' ) . esc_html( $e->getMessage() ) . '</p>';
			} catch ( Error $e ) {
				// Remove from rendering list on fatal error
				unset( $rendering_forms[ $form_id ] );
				return '<p class="gutena-forms-error">' . __( 'Fatal error rendering form: ', 'gutena-forms' ) . esc_html( $e->getMessage() ) . '</p>';
			}
		...
		/**
		 * Track form usage for synchronization
		 *
		 * @since 1.2.1
		 * @param int $form_id Form ID in custom post type.
		 */
		private function track_form_usage( $form_id ) {
			global $post;
			...
			// Get existing source posts
			$source_posts = get_post_meta( $form_id, '_gutena_forms_source_posts', true );
			if ( ! is_array( $source_posts ) ) {
				$source_posts = array();
			}
			
			// Add current post if not already tracked
			if ( ! in_array( $current_post_id, $source_posts, true ) ) {
				$source_posts[] = $current_post_id;
				update_post_meta( $form_id, '_gutena_forms_source_posts', $source_posts );
			}
		}
